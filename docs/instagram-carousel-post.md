...
- 그리드 안전영역: 중앙 `1012×1350` 3:4 크롭 기준
- 핵심 제목, 시리즈 번호, 브랜드명, 대표 그래픽은 모두 좌우 여백을 넉넉히 둔 중앙 영역에 배치
- 그리드 확인용 3:4 크롭: `docs/instagram-cards/grid-preview-3x4`
- 5장 그리드 배열 미리보기: `docs/instagram-cards/grid-preview-row.png`
- 프로필 그리드 확인용 크롭: `docs/instagram-cards/safe-crop-1x1`

## 디자인 기준

- 폰트: `Noto Sans KR`
- 톤: GPT 이미지 생성 무드를 참고한 인스타형 에디토리얼 포스터
- 구성: 4:5 캔버스 안에 중앙의 좁은 포스터형 콘텐츠 박스를 배치
- 콘텐츠 폭: 4:5 전체 폭을 쓰지 않고 중앙 약 `672px` 안쪽에 주요 텍스트와 그래픽 배치
- 목적: 피드에서는 4:5로 크게 보이고, 프로필 그리드에서는 좌우 크롭이 생겨도 핵심 제목과 대표 그래픽이 잘리지 않게 설계
- 색상: 국내증시 수급분석기의 녹색을 중심으로 하되, 카드별로 오렌지·청록·블루를 보조색으로 사용
